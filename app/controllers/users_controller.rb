  ...
  private
    ...
    def set_activity_counts
      @activity_counts = HashWithIndifferentAccess.new(
                          proposals: Proposal.where(author_id: @user.id).where(status: false).count,
    ...
    def load_proposals
      @proposals = Proposal.where(author_id: @user.id).where(status: false).order(created_at: :desc).page(params[:page])
    end
